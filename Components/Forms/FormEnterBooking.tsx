...
import { Autocomplete, AutocompleteItem, Avatar, Button, Input, Spinner, Divider, RadioGroup, Radio, cn, Card, CardHeader, CardBody, CardFooter } from "@nextui-org/react";
...
const FormEnter: React.FC<FormEnterProps> = ({ idcourse, idtitelwork, studentId }) => {
    ...
    const [stdidInput, setStdidInput] = useState(studentId)
    ...
    const getDataWorkOne = async () => {
        try {
            ...
            const responseone = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/work/typeone/${idcourse}`, {
                method: 'GET',
                headers: headers
            });
            if (!responseone.ok) {
    ...
    const statusButton = statusCheck;
    ...
    const handleClose = (event?: React.SyntheticEvent | Event, reason?: string) => {
        ...
        window.opener?.postMessage("grading_done", "*");
        window.close();
        setOpen(false);
    };
        ...
        <div className='flex justify-center mt-9'>
            ...
            <Card className="max-w-[300px]">
                <CardHeader className=' block pb-0'>
                    <p className='text-md'>{dataWorkOne?.name}</p>
                    <p className='text-sm text-gray-400'>{idcourse}</p>
                </CardHeader>
                <CardBody>
